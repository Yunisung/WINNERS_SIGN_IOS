//
//  ContractBusinessAgreeViewController.swift
//  WinnersSign
//
...
class ContractBusinessAgreeViewController : UIViewController, WKUIDelegate, UITextFieldDelegate, SignProtocol {
    ...
    // 서버에 이미 싸인이 등록됐을때는 저장을 안하기 위한 상태값
    ...
    override func viewDidLoad() {
        ...
        //계약서 이어서 쓸때
        if DataManager.instance.business.db_div == "U" {
    ...
    @IBAction func didTapHelp(_ sender: UIButton) {
        if let url = URL(string: "https://www.notion.so/11e33bf3c4134967b40666e649769a3e") {
...
//        }
        ...
        HttpsManager.instance.selectMyLicensee(view: self, idnum: DataManager.instance.sessionMember.mb_idnum, completionHandler: {
            ...
            if let data = data, let contract = try?
                ...
                if contract.result == "true" {
                    ...
                    self.upperTextField.text = contract.li_name
                    self.bottomTextField.text = contract.li_name
    ...
    func loadUpperWebView() {
        ...
        let weburl = URL(string: "https://winnerssign.bkwinners.kr/_files/li_agree1.html")!
    ...
    func loadBottomWebView() {
        ...
        let weburl = URL(string: "https://winnerssign.bkwinners.kr/_files/li_agree2.html")!
        let request = URLRequest(url: weburl)
    ...
    func SignData(fileName: String, data: Array<UInt8>) {
        ...
        if fileName == "\(DataManager.instance.sessionMember.mb_id)_li_sign1.png" {
            ...
            HttpsManager.instance.imgUpload(view: self, imagePath: fileName, fileGroup: "1", seq: DataManager.instance.business.li_seq, completionHandler: { (success, data) in
        ...
        if fileName == "\(DataManager.instance.sessionMember.mb_id)_li_sign2.png" {
            ...
            HttpsManager.instance.imgUpload(view: self, imagePath: fileName, fileGroup: "2", seq: DataManager.instance.business.li_seq, completionHandler: { (success, data) in
    ...
    @IBAction func didTapUpperSignButton(_ sender: UIButton) {
        let signView = self.storyboard?.instantiateViewController(withIdentifier: "SignDrowViewController") as? SignDrowViewController
        signView?.fileName = "\(DataManager.instance.sessionMember.mb_id)_li_sign1.png"
    ...
    @IBAction func didTapBottomSignButton(_ sender: UIButton) {
        let signView = self.storyboard?.instantiateViewController(withIdentifier: "SignDrowViewController") as? SignDrowViewController
        signView?.fileName = "\(DataManager.instance.sessionMember.mb_id)_li_sign2.png"
        signView?.modalPresentationStyle = UIModalPresentationStyle.fullScreen
        signView?.delegate = self
    ...
    @IBAction func didTapNext(_ sender: Any) {
        ...
        if DataManager.instance.business.db_div == "U" {
            ...
            dic.updateValue(upperTextField.text!, forKey: "li_name")
            dic.updateValue(DataManager.instance.business.li_seq, forKey: "li_seq")
            
            HttpsManager.instance.updateLicensee(view: self, data: dic, completionHandler: { (success, data) in
                
                let response = data as! String
                let data = response.data(using: .utf8)
                if let data = data, let contract = try? JSONDecoder().decode(Business.self, from: data) {
                    if contract.result == "true" {
                        //계약서 사진 저장
                        //DataManager.instance.business.li_seq = contract.li_seq
                        self.UploadImage()
            ...
            HttpsManager.instance.insertLicensee(view: self, idnum: DataManager.instance.sessionMember.mb_idnum, signName: upperTextField.text!, completionHandler: { (success, data) in
...
//        HttpsManager.instance.imgUpload(view: self, imagePath: "\(DataManager.instance.sessionMember.mb_id)_li_sign1.png", fileGroup: "1", seq: DataManager.instance.business.li_seq, completionHandler: { (success, data) in
//
//            let response = data as! String
//            let data = response.data(using: .utf8)
//            if let data = data, let result = try? JSONDecoder().decode(UploadImageResult.self, from: data) {
//                if result.result == "success" {
//
//                    HttpsManager.instance.imgUpload(view: self, imagePath: "\(DataManager.instance.sessionMember.mb_id)_li_sign2.png", fileGroup: "2", seq: DataManager.instance.business.li_seq, completionHandler: { (success, data) in
//
//                        let response = data as! String
//                        let data = response.data(using: .utf8)
//                        if let data = data, let result = try? JSONDecoder().decode(UploadImageResult.self, from: data) {
...
//        })
    ...
    func MoveNextScene() {
        //다음으로 이동
        ViewControllerManager.instance.ChangeViewController(viewController: self, nextViewController: "ContractBusinessServiceViewController")
    }
