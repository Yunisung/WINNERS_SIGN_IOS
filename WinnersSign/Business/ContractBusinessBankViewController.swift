//
//  ContractBusinessBankViewController.swift
//  WinnersSign
//
...
class ContractBusinessBankViewController : UIViewController, UITextFieldDelegate, SignProtocol {
    func SignData(fileName: String, data: Array<UInt8>) {
        let bytes: NSData = NSData(bytes: data, length: data.count)
        
        if fileName == "\(DataManager.instance.sessionMember.mb_id)_li_sign3.png" {
    ...
    @IBAction func didTapHelp(_ sender: UIButton) {
        if let url = URL(string: "https://www.notion.so/b8581f81919f46daa9fc0dbf9d9d4ed5") {
...
//        }
        ...
        HttpsManager.instance.selectMyLicensee(view: self, idnum: DataManager.instance.sessionMember.mb_idnum, completionHandler: {
            ...
            if let data = responseData, let contract = try?
                ...
                if contract.result == "true" {
                    ...
                    if contract.li_bank_name.isEmpty {
                        self.bankButton.setTitle("은행명 선택", for: .normal)
                    } else {
                        self.bankButton.setTitle(contract.li_bank_name, for: .normal)
                    }
                    
                    self.bankNumTextField.text = contract.li_bank_num
                    self.depositorTextField.text = contract.li_depositor
                    self.signTextField.text = contract.li_name
                    self.underTextField.text = contract.li_name
                    self.agentTextField.text = contract.li_agent
    ...
    @IBAction func didTapSign(_ sender: UIButton) {
        ...
        signView?.fileName = "\(DataManager.instance.sessionMember.mb_id)_li_sign3.png"
    ...
    func RequestServer() {
        ...
        dic.updateValue((bankButton.titleLabel?.text!)!, forKey: "li_bank_name")
        dic.updateValue(bankNumTextField.text!, forKey: "li_bank_num")
        dic.updateValue(depositorTextField.text!, forKey: "li_depositor")
        
        //담당자 임시 삭제
        //dic.updateValue(agentTextField.text!, forKey: "li_agent")
    ...
    func UploadImage() {
        ...
        let fileName = "\(DataManager.instance.sessionMember.mb_id)_li_sign3.png"
        
        HttpsManager.instance.imgUpload(view: self, imagePath: fileName, fileGroup: "7", seq: DataManager.instance.business.li_seq, completionHandler: {
            (success, data) in
    ...
    func NextViewController() {
        //다음씬
        ViewControllerManager.instance.ChangeViewController(viewController: self, nextViewController: "ContractBusinessFinishViewController")
    }
